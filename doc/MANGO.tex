...
MANGO is not designed to replace the meta-data already present in query responses, but on the contrary, to provide a model-aware layer with structured classes to interpret them and exploit them in client applications. 
...
These science usecases are detailed \ref{sec:chandra}.
...
The root class of the model is \texttt{MangoObject} : it has only
...
In addition to its identifier, \texttt{MANGOObject} instances have 2 components:
...
\begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt]
    ...
    \item \textbf{Semantics}: the semantic tag specifies the exact role of the property by
          ...
          For example, a signal amplitude associated with a time and position coordinates can be tagged
          as a photon event.
    \item \textbf{Description}: free text description.
    \item \textbf{Literal attributes} : some property classes embed qualifiers telling 
          how the quantity must be interpreted (e.g. colour vs hardness ratio) and labelled if plotted. 
\end{itemize}
...
To achieve this, table rows must be linked to the model using MIVOT annotations \citep{2023ivoa.spec.0620M}.
...
\begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt]
    \item MANGO as a Global View: This configuration enables full utilisation of the 
          ...
          This approach allows for serialization formats like XML or JSON, which often require
          a unique root.
...
The \emph{MANGOCore} table has one set of columns per Property class plus one column for the catalog ID.
